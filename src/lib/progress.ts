...
export function streamProgress(
  ...
  if (!reporter) {
    ...
    reporter = {
      mode: 'human',
      _write: (s: string) => process.stdout.write(s),
      syncProgress: (data: string) => process.stdout.write(`\r\x1b[K  \x1b[36m${data}\x1b[0m`),
      syncFileChange: (type: string, filename: string) => {
        if (type === 'delete') {
